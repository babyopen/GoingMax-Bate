...
const ViewCommon = {
  ...
  // ============================================================
  // 区域标签格式化：把「区域名」渲染成「区域名+次数区间」
  // 2026-08-14 新增：用户需求"标注每个区域实际的次数"
  // 例：12 期窗口的「冷号区」→「冷号区0」、「穿插区」→「穿插区1」
  // 2026-08-14 升级：用户进一步要求显示完整次数区间（如「冷号区0-1」、「封顶区8+」）
  // 数据来源：business-sliding-window.js 中 getZone6/12/24/36 的反向映射
  // 仅渲染格式化、不做业务计算（阈值表为常量查找表）
  // ============================================================

  /**
   * 区域名 → 该区域最低次数门槛 映射表（按窗口分组）
   *  - 6  期窗口（短窗口）：与 12/24/36 同区域共用颜色，因此不显示次数下限（业务语义是"出现 N 次"，无固定下限）
   *  - 12 期窗口：冷号区=0、穿插区=1、热号区=2、降权区=3、封顶区=4+
   *  - 24 期窗口：冷号区=0-1、穿插区=2、活跃区=3、热号区=4、过热区=5、降权区=6-7、封顶区=8+
   *  - 36 期窗口：冷号区=0-1、穿插区=2、活跃区=3-4、热号区=5-6、过热区=7-8、降权区=9-11、封顶区=12+
   *
   * 值为字符串时表示完整区间（含「下限- 上限」或「下限+」上限无穷）；
   * 值为数字时表示仅记录下限（用于 12 期窗口等单值映射）。
   */
  ZONE_RANGE_MAP: Object.freeze({
    '12': { '冷号区': 0, '穿插区': 1, '热号区': 2, '降权区': 3, '封顶区': '4+' },
    '24': { '冷号区': '0-1', '穿插区': 2, '活跃区': 3, '热号区': 4, '过热区': 5, '降权区': '6-7', '封顶区': '8+' },
    '36': { '冷号区': '0-1', '穿插区': 2, '活跃区': '3-4', '热号区': '5-6', '过热区': '7-8', '降权区': '9-11', '封顶区': '12+' }
  }),

  /**
   * 生成"区域 + 次数区间"的显示文本
   * @param {string} zone - 区域名（如 '冷号区' / '穿插区'）
   * @param {number|string} window - 窗口期数（6 / 12 / 24 / 36，传字符串或数字均可）
   * @returns {string} 形如 '冷号区0-1' / '穿插区2' / '封顶区8+'；窗口不支持或区域不在映射表时回退为原区域名
   *
   * @example
   *   ViewCommon.getZoneLabel('冷号区', 12);  // '冷号区0'
   *   ViewCommon.getZoneLabel('冷号区', 24);  // '冷号区0-1'
   *   ViewCommon.getZoneLabel('封顶区', 24);  // '封顶区8+'
   *   ViewCommon.getZoneLabel('穿插区', 36);  // '穿插区2'
   *   ViewCommon.getZoneLabel('短冷号', 6);   // '短冷号'（6 期窗口未提供次数区间）
   */
