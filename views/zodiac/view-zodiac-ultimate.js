...
const ViewZodiacUltimate = {
  ...
  renderUltimateAlgorithm: function(data) {
    const resultContainer = document.getElementById('ultimateResultContainer');
    const expectDisplay = document.getElementById('ultimateExpectDisplay');
    ...
    if (report.currentStage === '数据不足无法判断') {
      const adviceText = report.cycleStatus && report.cycleStatus.advice ? report.cycleStatus.advice : '历史数据不足，无法准确判断周期';
    ...
    const adaptiveInfo = data.adaptiveInfo || {};
    const mainCount = adaptiveInfo.mainCount || 5;
    const backupCount = adaptiveInfo.backupCount || 3;
    const isAdaptive = adaptiveInfo.isAdaptive || false;
    ...
    if (report.currentStage === '过渡混沌期') {
      ...
      if (data.numbers) {
        data.numbers.forEach(function(item, idx) {
          const rank = idx + 1;
          const rankClass = rank === 1 ? 'card-rank-1' : (rank === 2 ? 'card-rank-2' : (rank === 3 ? 'card-rank-3' : 'card-rank-other'));
          const emoji = ZodiacPrediction.getZodiacEmoji(item.zodiac);
    ...
    const stageInfo = report.cycleStatus;
  ...
  _readV1FromDOM: function() {
    ...
    if (!grid) return [];
    const visiblePanel = grid.querySelector('.freq-panel[style*="display: block"], .freq-panel:not([style*="display: none"])');
    const target = visiblePanel || grid;
    const cards = target.querySelectorAll('.zodiac-static-card .zodiac-static-name');
    const list = [];
  ...
  _readV2FromDOM: function() {
    const panel = document.getElementById('giongRecommendPanel');
    if (!panel) return [];
    const cards = panel.querySelectorAll('.zodiac-static-card .zodiac-static-name');
    const list = [];
    cards.forEach(function(el) { if (el.textContent) list.push(el.textContent.trim()); });
    return list;
  },
  ...
  _readUltimateFromDOM: function() {
    const container = document.getElementById('ultimateResultContainer');
    if (!container) return [];
    const names = container.querySelectorAll('.db-card-name');
    const list = [];
  ...
  renderUnrecommendedZodiacs: function(_) {
    const ultimateResultContainer = document.getElementById('ultimateResultContainer');
    if (!ultimateResultContainer) return;
    const cardBody = ultimateResultContainer.parentNode;
    if (!cardBody) return;

    let panel = document.getElementById('unrecommendedZodiacPanel');
    if (!panel) {
    ...
    const v1List = ViewZodiacUltimate._readV1FromDOM();
    const v2List = ViewZodiacUltimate._readV2FromDOM();
    const ultimateList = ViewZodiacUltimate._readUltimateFromDOM();

    const ZODIAC_ORDER = ZodiacPrediction.ZODIAC_ORDER;
    const ZODIAC_EMOJI = ZodiacPrediction.ZODIAC_EMOJI;
    ...
    const v1Uniq = uniq(v1List);
    const v2Uniq = uniq(v2List);
    const ultUniq = uniq(ultimateList);
    const allRecSet = {};
    [].concat(v1Uniq, v2Uniq, ultUniq).forEach(function(z) { allRecSet[z] = true; });
    const allRecommended = Object.keys(allRecSet);
  ...
  showUnrecSourcesModal: function() {
    const src = ViewZodiacUltimate._lastUnrecSources;
    if (!src) {
      Toast.show('暂无来源数据');
      return;
    }
  ...
  renderUltimateBacktest: function(summary, currentBackupCount) {
    const container = document.getElementById('ultimateBacktestContainer');
    if (!container) return;
    ...
    const totalHitClass = summary.totalHitRate >= 60 ? 'backtest-rate-high' : (summary.totalHitRate >= 35 ? 'backtest-rate-mid' : 'backtest-rate-low');
    const adaptiveState = BusinessUltimate.getAdaptiveState();

    const actualMainCount = 4;
    const actualBackupCount = currentBackupCount || adaptiveState.currentBackupCount || 3;
    ...
    if (summary.missTotalNotInRecommend !== undefined || summary.missInBlackList !== undefined) {
      const totalMiss = summary.total - (summary.totalHits || summary.hits);
      detailHtml += '<div class="backtest-miss-analysis">';
      detailHtml += '<div class="backtest-miss-title">未命中原因分析（基于主推+备选）：</div>';
      if (totalMiss > 0) {
        const missBlackPct = Math.round((summary.missInBlackList || 0) / totalMiss * 100);
        const missNotRecPct = Math.round((summary.missTotalNotInRecommend || 0) / totalMiss * 100);
    ...
    recentRecords.forEach(function(r) {
      ...
      let topNText = r.topN.join(' ');
      if (r.hit && r.hitRank >= 1 && r.hitRank <= r.topN.length) {
        const _hitIdx = r.hitRank - 1;
      ...
      const stageTag = r.stage ? '<span class="backtest-stage-tag">' + r.stage.replace('稳定运行期', '').replace('过渡混沌期', '过渡') + '</span>' : '';
      const blackInfo = r.blackListCount > 0 ? '<span class="backtest-black-info">降权' + r.blackListCount + '个</span>' : '';
